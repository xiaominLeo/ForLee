<template>
  <div class="content">
    <router-link id="love" to="/LoveYou">
      <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" t="1539851347959" class="icon" style="" viewBox="0 0 1025 1024" version="1.1" p-id="4260" width="200.1953125" height="200"><path d="M1024.549 360.609c0-170.492-133.815-309.265-298.055-309.265-81.129 0-157.91 34.998-213.344 94.701-55.509-59.702-132.367-94.701-213.344-94.701C135.49 51.344 1.751 190.041 1.751 360.609c0 5.719 0.534 10.827 0.991 15.021-0.076 1.373-0.152 2.745-0.152 4.194 0 30.193 7.319 63.361 21.73 98.59 0.458 1.295 0.915 2.516 1.449 3.657 90.812 217.844 440.412 468.474 455.279 479.985 9.227 7.092 20.205 10.6 31.263 10.6 11.209 0 22.266-3.659 31.566-10.903 12.733-9.911 310.941-224.551 429.279-427.603 4.498-6.861 7.854-13.494 10.828-19.215 0.914-1.829 1.753-3.658 2.744-5.413l0.382-0.839c0.382-0.686 0.839-1.449 1.296-2.059 7.091-13.802 12.732-26.611 17.232-39.116 12.274-32.177 18.3-60.847 18.3-87.61 0-2.058-0.077-3.888-0.229-5.414C1024.093 370.979 1024.549 366.251 1024.549 360.609z" p-id="4261" fill="#f075ae"/></svg>
    </router-link>
    <div class="title">
      {{title}}
    </div>
    <ul>
      <li class="days">
        <span>{{days}}</span>
        <span>Days</span>
      </li>
      <li class="hours">
        <span>{{hours}}</span>
        <span>Hours</span>
      </li>
      <li class="minutes">
        <span>{{minutes}}</span>
        <span>Minutes</span>
      </li>
      <li class="seconds">
        <span>{{seconds}}</span>
        <span>Seconds</span>
      </li>
    </ul>

  </div>

</template>

<script>
export default {
  name: 'Welcome',
  data () {
    return {
      days:'',
      hours:'',
      minutes:'',
      seconds:'',
      title:"We've known each other For",
      items: [{name:"item1",id:1},{name:"item2",id:2},{name:"item3",id:3},{name:"item4",id:4}]
    }
  },
  created() {
      let that = this;
      setInterval(function() {
        that.getTimes();
      },1000)
  	},
  methods: {
      getTimes() {
        let that = this;
        let currentTime = new Date().getTime();
        let startTime = new Date("2017-10-18").getTime();
        // console.log(currentTime)
        let diffTime = currentTime - startTime;
        that.days = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
        that.hours = Math.ceil(diffTime / (1000 * 60 * 60));
        that.minutes = Math.ceil(diffTime / (1000 * 60));
        that.seconds = Math.ceil(diffTime / 1000);
        // console.log(that.seconds)
        // console.log(that.days +"Days-"+ that.hours +"Hours-"+ that.minutes +"Minutes-"+ that.seconds +"Seconds");
      },

  },
  mounted() {

  }
}
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped lang="less">
  .content{
    width:800px;
    margin:30px auto;
    .title{
      color:#a6ade4;
      font-size: 50px;
      margin-top: 30px;
    }
    ul{
      li{
        list-style:none;
        text-align:center;
        margin-top:20px;
        span:first-child{
          font-size: 40px;
        }
        span:last-child{
          font-size: 56px;
        }
      }
      .days{
        span:first-child{
          color:#9dd8e0;
        }
        span:last-child{
          color:#f2d7e1;
        }
      }
      .hours{
        span:first-child{
          color:#e6b6b0;
        }
        span:last-child{
          color:#b0e6d1;
        }
      }
      .minutes{
        span:first-child{
          color:#81f0e9;
        }
        span:last-child{
          color:#eeb173;
        }
      }
      .seconds{
        span:first-child{
          color:#a6ade4;
        }
        span:last-child{
          color:#7dc5eb;
        }
      }
    }
  }

</style>
